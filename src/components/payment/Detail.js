  ...
  return (
    <div className=" flex flex-col bg-white rounded-xl justify-center items-center content-start p-5 lg:mt-0 md:mt-0 mt-10">
      <div className="w-[200px] lg:w-[400px] md:w-[400px] sm:w-[400px]">
        ...
        <p className="font-light text-sm">{handlerDate(dataSchedule?.departureDate)}</p>
        ...
        <div className="flex flex-row items-center gap-3 border-t-2 border-b-2 my-3">
          ...
          <div className=" my-3 flex flex-col gap-3">
            <div>
              <p className="font-bold text-sm">{dataSchedule?.plane?.airline?.airlineName} - {dataSchedule?.class?.name}</p>
              <p className="font-bold text-sm">{dataSchedule?.plane?.airline?.airlineCode}-{dataSchedule?.plane?.id}</p>
            </div>
            <div>
              <p className="font-bold text-sm">Informasi :</p>
              <p className="text-sm">Baggage {dataSchedule?.plane?.baggageMaxCapacity} kg</p>
              <p className="text-sm">Cabin Baggage {dataSchedule?.plane?.cabinMaxCapacity} kg</p>
            ...
            dataSeat?.adult > 0 && (
              <>
                <div className="flex flex-row justify-between">
                  ...
                  <p className="text-sm">{handlerIDR(dataSchedule?.adultPrice)}</p>
                </div>
              </>
            )
          }
        ...
        <div className="flex justify-between items-center">
          ...
          <p className="font-bold text-lg text-purple-900 my-3">
            {handlerIDR((dataSeat?.adult * dataSchedule?.adultPrice) + (dataSeat?.child * dataSchedule?.kidsPrice) + (dataSeat?.baby * dataSchedule?.babyPrice) + (dataSchedule?.taxPrice))}
          </p>
        </div>
