...
export default function AccordionHistory({ item }) {
  ...
  const [isLoadingClickButtonBatal, setIsLoadingClickButtonBatal] =
    useState(false)
  ...
  const handleClickButtonBatal = async () => {
    ...
    Swal.fire({
      ...
      text: "Apakah anda ingin membatalkan pemesanan tiket?",
      focusConfirm: false,
      focusCancel: false,
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.put(
            `${baseUrl}/v1/api/order/cancel-payment`,
            {
  ...
  const handleClickLanjutBayar = async (orderId) => {
    Swal.fire({
      title: "Pemesanan Tiket",
      text: "Apakah Anda ingin melanjutkan pemesanan tiket?",
      showCancelButton: true,
      confirmButtonText: "Ya",
      cancelButtonText: "Tidak",
      confirmButtonColor: "#16a34a",
            ...
            <div className=" p-4 border-t-2 w-11/12 border-slate-300">
              ...
              <div className="flex md:justify-end justify-center mt-5 md:gap-6 gap-3">
                ...
                {item?.order.payment[0].status == "Unpaid" && (
                  <>
                    <button
                      onClick={() => handleClickLanjutBayar(item.order.id)}
                      className=" md:px-16 px-7 py-3 rounded-2xl hover:bg-[#16a34a] bg-[#32CD32] text-white"
                    >
                      Lanjut Bayar
                    </button>
                    {isLoadingClickButtonBatal ? (
                      ...
                      <button
                        onClick={handleClickButtonBatal}
                        className=" md:px-16 px-7 py-3 rounded-2xl bg-orange-500 hover:bg-orange-400 text-white"
                      >
                        Batal
                      </button>
                    )}
