...
const ImportDateRow = ({ startDate, endDate, setIsFilter }) => {
  ...
  const getRange = (startData, endData) => {
    ...
    if (endData) {
      ...
      setDateRange([
        {
          date: `${getDate}-${getMonth + 1}-${getYear}`,
          day: getDay,
          search: `${getYear}-0${getMonth + 1}-0${getDate}`,
        },
      ])
    }
  }
  ...
  return (
    <>
      <div
        className={`${styles.mainRow} flex gap-3 text-xs lg:text-base md:text-base xstext-xs:text-sm`}
      >
        {dateRange?.map((item, i) => (
          <div
            className="flex flex-row items-center justify-center gap-4 h-[80px]"
            key={i}
          >
            <button
              onClick={() => hanClick(item?.search)}
              className={
                "flex flex-col items-center hover:bg-orange-300  active:bg-orange-500 focus:bg-orange-500 py-2 hover:rounded-xl focus:rounded-xl focus:text-white text-black hover:text-white duration-75 px-5"
              }
            >
              <div className="font-bold ">
                {item?.day !== "Invalid Date" ?? ""}
              </div>
