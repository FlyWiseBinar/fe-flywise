...
const MainOrderHistory = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${api.apiHistoryByOrderCode}?orderCode=${searchQuery}`,
        {
          headers: {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const response = await axios(api.apiHistory, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
    ...
    if (value) {
      ...
      try {
        const response = await axios(
          `${api.apiHistoryByFilterHistory}?startDate=${value.startDate}&endDate=${value.endDate}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    <div className="md:mb-16">
      ...
      <div className="flex flex-row justify-center items-center md:w-full w-full md:gap-3 gap-2">
        ...
        <button
          ...
          className="text-xl text-main-purple"
        >
          <LuRotateCw />
        </button>
                ...
                >
                  <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                    ...
                    >
                      ...
                      <button
                        type="submit"
                        className="bg-main-purple hover:bg-second-purple text-white rounded-lg p-2 text-xl"
                      >
                        <AiOutlineSearch />
                      </button>
                    </form>
