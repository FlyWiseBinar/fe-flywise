...
const MainOrderHistory = () => {
  ...
  const fetchFilteredDate = async (value) => {
    try {
      const response = await fetch(`https://be-flywise-stagging-jcbxz3zpbq-as.a.run.app/v1/api/order/historyfilter?startDate=${value.startDate}&endDate=${value.endDate}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleSubmitDate = async (value) => {
    if (value) {
      await fetchFilteredDate(value)
    }
  }
  ...
  return (
    <div className="md:mb-16">
      ...
      <Button onSearch={handleSearch} onSubmit={handleSubmitDate} />
      {showAlert && (
        <div>
